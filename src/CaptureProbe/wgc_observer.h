#pragma once
// CaptureProbe/wgc_observer.h — P3 并行 FP16 观察池（无损：不接触 SDK 路径的任何对象）
//
// 原理：自有 D3D11 设备 → WGC CreateFreeThreaded(FP16) 池 → 对主显示器开自己的
//       CaptureSession → 轮询每帧：SystemRelativeTime(fps) + CopyResource 干跑
//       （§7.1 步骤 4 的等尺寸纹理机制验证）+ 节流 FP16 像素统计
//       （overWhite = max(R,G,B)>1.0 占比，量化 BGRA 路径丢失的 HDR 高光）。
//
// 与 P2 的 BGRA 路径采样对照：FP16 overWhite% vs 客户端 BGRA bright%@250。
